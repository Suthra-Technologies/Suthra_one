...
public class PrintStationService extends Service {
    ...
    // ── Printing ────────────────────────────────────────────────────────
    private void printOrder(JSONObject bill) throws Exception {
        // KOT first, then bill — matching the foreground behavior.
        if ("epos-print".equals(commandMode)) {
            sendEpos(EscPosFormatter.buildKotEpos(bill));
            sendEpos(EscPosFormatter.buildBillEpos(bill));
        } else {
            sendRaw(EscPosFormatter.buildKot(bill));
            sendRaw(EscPosFormatter.buildBill(bill));
        }
    }
