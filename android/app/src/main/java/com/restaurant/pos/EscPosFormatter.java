...
public final class EscPosFormatter {
    ...
    // Currency prefix for the current bill, set per build call. Printer-safe (no ₹/€/£ glyphs).
    ...
    /** Map a currency code to a printer-safe prefix (thermal printers lack ₹/€/£/¥ glyphs). */
    private static String currencyPrefixFor(JSONObject bill) {
        ...
        switch (code) {
            ...
            case "CAD": return "C$";
            case "SGD": return "S$";
            default: return code + " "; // e.g. "JPY 100.00", "CNY 50.00"
        }
    }
    ...
    /** Restaurant timezone from the bill's restaurant object (defaults to America/New_York). */
    ...
    /** Parse the UTC ISO timestamp and format it in the restaurant timezone, US format. */
    ...
    private static String[] resolveQr(JSONObject bill, JSONObject r) {
        ...
        if (isDelivery) {
            ...
            String handoff = bill.optString("handoffQr", "");
            if (handoff != null && !handoff.isEmpty() && !"null".equals(handoff)) {
                return new String[]{ handoff, "Driver: Scan to Confirm Pickup" };
            }
        ...
        return new String[]{ "https://nexzenpos.com/" + slug + "/feedback/" + id, "Scan to Rate Us" };
    }
    ...
    static Logo buildLogo(String url) {
        ...
        // Auto-crop blank/transparent margins so there is no big gap and the logo isn't clipped.
    ...
    public static byte[] buildBill(JSONObject bill) {
        ...
        e.raw(AC).raw(BOLD_ON).raw(DBL_H).line(r != null ? r.optString("name", "Restaurant") : "Restaurant").raw(DBL_OFF).raw(BOLD_OFF);
        if (r != null) {
        ...
        if (disc > 0) e.leftRight("Discount:", "-" + money(disc));
        if (bill.optDouble("rewardDiscount", 0) > 0) e.leftRight("Points Discount:", "-" + money(bill.optDouble("rewardDiscount")));
        if (bill.optDouble("tip", 0) > 0) e.leftRight("Tip:", money(bill.optDouble("tip")));
        e.rule();
    ...
    public static String buildBillEpos(JSONObject bill) {
        ...
        JSONObject tax = bill.optJSONObject("tax");
        double taxAmt = (tax != null ? tax.optDouble("amount", 0) : 0) + bill.optDouble("processingFee", 0);
        if (taxAmt > 0) p.append(t(lr("Tax & Fees:", money(taxAmt))));
        if (bill.optDouble("deliveryCharge", 0) > 0) p.append(t(lr("Delivery:", money(bill.optDouble("deliveryCharge")))));
        JSONObject sc = bill.optJSONObject("serviceCharge");
        if (sc != null && sc.optDouble("amount", 0) > 0) p.append(t(lr("Service Charge:", money(sc.optDouble("amount")))));
        double disc = discountAmount(bill);
        if (disc > 0) p.append(t(lr("Discount:", "-" + money(disc))));
        if (bill.optDouble("rewardDiscount", 0) > 0) p.append(t(lr("Points Discount:", "-" + money(bill.optDouble("rewardDiscount")))));
        if (bill.optDouble("tip", 0) > 0) p.append(t(lr("Tip:", money(bill.optDouble("tip")))));
        p.append(t(repeat('-', W)));
        boolean paid = "paid".equals(bill.optString("paymentStatus", ""));
